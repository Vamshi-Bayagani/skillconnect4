...
def recruiter_dashboard(request):
    ...
    assignments = Assignment.objects.filter(
        ...
        status="submitted"   # ✅ only submitted ones
    ).order_by("-created_at")
...
def assignment_create(request):
    ...
    if request.method == "POST":
        ...
        if freelancer.email:
            send_mail(
                subject="📌 New Assignment Assigned to You – SkillConnect",
                message=(
                    ...
                    f"• Assigned by: {recruiter.first_name or 'Recruiter'}\n\n"
                    "🔐 Please log in to your SkillConnect dashboard to view and submit the assignment before the deadline.\n\n"
        ...
        messages.success(request, "Assignment created and freelancer notified 📧")
...
def assignment_submit(request, assignment_id):
    ...
    if request.method == "POST":
        ...
        if recruiter.email:
            send_mail(
                subject="📥 Assignment Submitted – SkillConnect",
                message=(
                    f"Hi {recruiter.first_name or 'Recruiter'},\n\n"
                    f"The assignment '{assignment.title}' has been submitted.\n\n"
                    f"👤 Freelancer: {request.user.first_name}\n"
                    f"💼 Job: {assignment.job.title}\n"
                    f"📅 Submitted on: {assignment.created_at.strftime('%d %b %Y')}\n\n"
